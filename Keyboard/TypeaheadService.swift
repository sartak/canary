...
class TypeaheadService {
    ...
    init?(db: OpaquePointer, maxSuggestions: Int = 20) {
        ...
        let commonWordsQuery = "SELECT word, hidden FROM words ORDER BY frequency_rank LIMIT \(maxSuggestions)"
    ...
    func getCompletions(prefix: String, suffix: String) -> (completions: [String], exactMatch: String?) {
        ...
        let targetWord = !prefix.isEmpty || !suffix.isEmpty ? (prefix + suffix).lowercased() : nil

        while sqlite3_step(statement) == SQLITE_ROW {
            if let wordPtr = sqlite3_column_text(statement, 0) {
                let word = String(cString: wordPtr)
                let isHidden = sqlite3_column_int(statement, 1) == 1
                let isExactMatch = targetWord != nil && word.lowercased() == targetWord

                if isExactMatch && exactMatch == nil {
                    exactMatch = word
                }
